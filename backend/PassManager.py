...
def getFernetKey(): #gets firnet key from the zip file without unzipping it
	inputpass = input("Input Password to Make Changes: ")
	try:
...
def createPasswordTable():
	...
	encrypt("Vault.csv", getFernetKey()) #encrypt the Vault

def addToPasswordTable():
	decrypt("Vault.csv", getFernetKey())#decrypt Vault
	...
	pd.DataFrame(table).to_csv("Vault.csv",index=False)
	encrypt("Vault.csv", getFernetKey())#then encrypt it again

def viewPasswordTable():
	decrypt("Vault.csv", getFernetKey())#decrypt Vault
	df = pd.read_csv (r"Vault.csv")
	table = df.values.tolist()
	print(table)
	encrypt("Vault.csv", getFernetKey())#encrypt vault
...
def returning():
	...
	while ask == True:
		...
		if "1" in options:
			viewPasswordTable() #view the table of passwords
		if "2" in options:
			addToPasswordTable() #add to the table of passwords
		if "3" in options:
			ask = False
			print("Goodbye!")
...
initialSetup()#this should only be run on forst startup
returning()#this should only be run after the first startup has been creates sucessfully
